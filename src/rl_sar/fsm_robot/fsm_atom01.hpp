...
#ifndef ATOM01_FSM_HPP
#define ATOM01_FSM_HPP

#include "fsm.hpp"
#include "rl_sdk.hpp"

namespace atom01_fsm
{
...
public:
    ...
    void Enter() override
    {
        ...
        try
        {
            ...
            // ATOM01 has no observation history stacking (history_length=1),
            // so no need to reset history_obs_buf here.
        }
...
class ATOM01FSMFactory : public FSMFactory
{
public:
    ATOM01FSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<atom01_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<atom01_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<atom01_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLNioLabLocomotion")
            return std::make_shared<atom01_fsm::RLFSMStateRLNioLabLocomotion>(rl);
        return nullptr;
    }
    std::string GetType() const override { return "ATOM01"; }
...
REGISTER_FSM_FACTORY(ATOM01FSMFactory, "RLFSMStatePassive")

#endif // ATOM01_FSM_HPP
